                ...
                @foreach ($tasks as $task)
                    <div class="bg-gray-800 text-white shadow-md rounded-lg p-6">
                        ...
                        <div class="flex space-x-2 mt-4">
                            <!-- Botón para Editar -->
                            @if($user->id == $task->user_id || (!$task->sharedWith()->where('user_id', $user->id)->get()->isEmpty() && $task->sharedWith()->where('user_id', $user->id)->first()->pivot->permissions == 'edit'))
        ...
        <div @click.outside="modalShare = false" class="max-h-full w-full max-w-xl overflow-y-auto sm:rounded-2xl bg-white">
            <div class="w-full p-6">
                ...
                <div class="mb-4">
                    ...
                    <select wire:model="selectedUserId" class="w-full p-2 border rounded">
                ...
                <button wire:click="addShareWithUser" class="px-4 py-2 bg-yellow-600 text-white rounded">Añadir</button>

                <!-- Lista de Usuarios y Permisos Seleccionados -->
                <ul class="mt-4">
                    @foreach($shareWith as $shareWithElem)
                        @php
                            $user = $users->where('id', $shareWithElem['userId'])->first();
                        @endphp
                        <li class="flex justify-between items-center py-2">
                            <span>{{ $user->name ?? 'Usuario desconocido' }} ({{ ucfirst($shareWithElem['permissions']) }})</span>
                        </li>
                    @endforeach
                </ul>
